...
class GradTTS(BaseModule):
    ...
    def forward(self, x, x_lengths, bert, n_timesteps, temperature=1.0, stoc=False, spk=None, length_scale=1.0, use_diff=1):
        ...
        """
        x, x_lengths, bert = self.relocate_input([x, x_lengths, bert])
        ...
        mu_x, logw, x_mask = self.encoder(x, x_lengths, bert, spk)
        ...
        encoder_outputs = encoder_outputs + torch.randn_like(encoder_outputs) * 0.25  # Perturbation
        return encoder_outputs, decoder_outputs, attn[:, :, :y_max_length]

    def compute_loss(self, x, x_lengths, bert, y, y_lengths, spk=None, out_size=None, skip_diff=False):
        """
        Computes 3 losses:
        ...
        """
        x, x_lengths, bert, y, y_lengths = self.relocate_input([x, x_lengths, bert, y, y_lengths])
